...
# ╔═╡ 9f6f37e8-478d-11f0-1e8a-d1f7765d3a98
using CairoMakie, JLD2, Interpolations, Polynomials, LsqFit, DifferentialEquations, DSP, PlutoUI

# ╔═╡ b21b72eb-91ee-4601-ba8c-a3bc1f606139
include("polariton_funcs.jl")

# ╔═╡ 54f13997-28d7-4d93-b6c5-f04a659c8d93
function model(x, p)
    y₋, y₊, x₀, k = p
    @. tanh(k * (x - x₀)) * (y₊ - y₋) / 2 + (y₊ + y₋) / 2
end

# ╔═╡ a7c3c8fc-ce2e-4010-a236-f63d17128f9c
...
# ╔═╡ fed5b1f0-daf1-4b12-8d48-fe1442383214
...
# ╔═╡ c8ec1c65-cc4c-45be-b696-5f9533a71b9e
...
# ╔═╡ a966129d-e06c-44f0-8cee-eec7337c5ef9
...
# ╔═╡ f42b8ab1-343e-4565-bfd0-b2a20ff45d1c
saving_dir = "/home/marcsgil/Code/LEON/MomentumCorrelations/full_sim2/"

# ╔═╡ 946ba4cf-65f5-4102-a8ac-20717275bbbd
...
# ╔═╡ 16f88510-5da7-4d19-8a5c-6405ea2181e7
n_itp, k_itp, c_itp = build_interpolations(steady_state, param, -30, 30)

# ╔═╡ e7e6031f-e20a-4443-b583-1bc3cd587b68
xs_sim = (StepRangeLen(0, param.dx, param.N - 1) .- param.x_def)

# ╔═╡ 69b11ee8-5542-41e0-aa7b-b9e362025233
ns = abs2.(steady_state[1][2:end])

# ╔═╡ 493aae17-c83d-4432-ac46-6ee3773c07a9
ks = diff(unwrap(angle.(steady_state[1]))) / param.dx

# ╔═╡ 9c3216f8-dd4d-4c5e-b96d-689c4debd86b
cs = map((n, k) -> speed_of_sound(n, param.g, param.δ₀, k, param.ħ, param.m), ns, ks)

# ╔═╡ 532bdfb2-c38e-4f37-ba0d-38ab957373b6
begin
	δω = 0.65
...
# ╔═╡ 934df000-3cd5-48f6-9312-d2e166f455d9
@bind x PlutoUI.Slider(-7:0.01:2)

# ╔═╡ 21dd771e-d3ab-4c98-ba9f-ae03a0af2846
begin
	function plot_disperison(qs, n, K, param)
		δω₊ = [dispersion_relation(q, n, param.g, param.δ₀, K, param.ħ, param.m, true) for q ∈ qs]
		δω₋ = [dispersion_relation(q, n, param.g, param.δ₀, K, param.ħ, param.m, false) for q ∈ qs]
	
		with_theme(theme_latexfonts()) do
			fig = Figure(fontsize=24)
			ax = Axis(fig[1,1], xlabel=L"q \ (\mu \text{m}^{-1})", ylabel=L"\delta \omega \ (ps^{-1})", title = L"x = %$x")
			ylims!(ax, -1, 1)
			lines!(ax, qs, δω₊, linewidth=4)
			lines!(ax, qs, δω₋, linewidth=4)
			fig
		end
	end
	qs = LinRange(-1.5,1.5,512)
	
	plot_disperison(qs, n_itp(x), k_itp(x), param)
end
...
manifest_format = "2.0"
project_hash = "ae8b0809c91c7763ba5f24da858d3f3872ac60db"

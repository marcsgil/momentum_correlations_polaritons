...
# ╔═╡ 5589a01e-8c5c-4a40-a618-f9bdd3924073
using CairoMakie, JLD2, LinearAlgebra, DSP, KernelAbstractions

# ╔═╡ da8d63a1-2229-4c76-82cb-44f5788ac2bb
include("polariton_funcs.jl")

# ╔═╡ 92d070da-c140-4f69-a3c1-640d4edb2662
include("equations.jl")

# ╔═╡ e59882c5-6c7d-47ef-af47-c2a7ab6794d0
...
# ╔═╡ 8f8f6197-6c14-4294-beeb-22701b53e64c
...
# ╔═╡ 63f5a5f3-0ece-4e39-bd36-4853165fbd07
saving_dir = "/Users/marcsgil/Code/momentum_correlations_polaritons/data/"

# ╔═╡ 26ef512f-d3da-4e24-b3f1-42e975fbecc2
...
# ╔═╡ 9d1e0f8a-39d0-454f-bbb7-3dd7a00f5d5b
xs = (StepRangeLen(0, param.dx, param.N - 1) .- param.x_def)[2:end]

# ╔═╡ 6653ba23-9747-4602-952f-8e23057e12ed
idx_min = findfirst(x -> x >= -300, xs)

# ╔═╡ fa28300a-31c0-432d-88be-8ef2329d0fc3
idx_max = findlast(x -> x <= 300, xs)

# ╔═╡ e2db8828-ae4a-41b0-aa56-0448f29956e8
idxs = idx_min:idx_max

# ╔═╡ 9222fd9d-5512-4eeb-8b32-284f476daf4d
gns = param.g * abs2.(steady_state[1][2:end])[idxs]

# ╔═╡ 32626ba9-6353-4e8c-b1f8-623b5535eeee
ks = diff(unwrap(angle.(steady_state[1])))[idxs] / param.dx

# ╔═╡ cc3bdf7f-ac35-40a5-9415-231426cf7bc3
vs = param.ħ * ks / param.m

# ╔═╡ 4818a720-e41e-49dc-9bcf-c99b417202bb
δs = [detuning(param.δ₀, k, param.ħ, param.m) for k in ks]

# ╔═╡ 473107c5-71ae-4d62-abec-1f401863582b
Vs = [potential(x, param) for x ∈ xs[idxs] .+ param.x_def]

# ╔═╡ dd4bc6fd-53ce-429f-969c-4a31dac9c414
Γ = vcat(0, diff(vs))

# ╔═╡ ff96d1bc-efbd-4214-875d-f8d9a7e7a286
N  = length(gns)

# ╔═╡ 2029db95-51d4-4042-8952-c48ee5ac2453
∇ = first_derivative_operator(N, param.dx)

# ╔═╡ c99a751f-f21a-49f7-9d7d-a39a904d2b43
∇² = second_derivative_operator(N, param.dx)

# ╔═╡ 36b08ac2-6ff5-493c-8100-964d034769db
M11 = diagm(2gns - δs + Vs + im * Γ / 2) - im * diagm(vs) * ∇ - param.ħ * ∇² / 2param.m

# ╔═╡ 8213b007-83e0-4608-b626-498d50be969c
M22 = diagm(2gns - δs + Vs - im * Γ / 2) + im * diagm(vs) * ∇ - param.ħ * ∇² / 2param.m

# ╔═╡ de11c7a7-526f-44d4-8405-0474bb7d2def
M12 = diagm(gns)

# ╔═╡ da34f1c3-d0cb-4b70-94cc-04a4fd876567
M = stack([M11 M12; M12 M22])

# ╔═╡ c6263ed4-eebb-4589-9bdd-b34a1bc77bbd
vals, vecs = eigen(M)

# ╔═╡ d10f0083-5629-424c-ab59-59a8d849e4b9
begin
	k = 119
	fig = Figure()
	ax = Axis(fig[1,1])
	#lines!(ax, xs[idxs], abs.(vecs[1:N, k]))
	lines!(ax, xs[idxs], angle.(vecs[N+1:2N, k]))
	fig
end
...
manifest_format = "2.0"
project_hash = "e7027d41d779e41853bbc18be3bd4fae6d049003"
